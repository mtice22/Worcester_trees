...
Considering the audience, however, these metrics are hollow without an explicit value. i-Tree is a resource that provides a carbon offset metric, as well as a cost benefit for this offset. However, without the DBH data for Burncoat/Greendale, the total cost benefit is unattainable. On the other hand, Trees Atlanta is an urban forestry website with a “Carbon Offset Calculator.” This calculator uses the total number of trees to make a rough estimate of carbon offset and the corresponding cost benefit. While i-Tree would have provided a more accurate value, Trees Atlanta’s calculator provided estimate values that place a concrete value to trees’ impact on air quality.
